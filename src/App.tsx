...
import { DEFAULT_AGENTS, MODELS} from './constants';
    ...
    <div className="flex h-screen w-full overflow-hidden font-sans text-black dark:text-white transition-colors duration-300">
      ...
      {editingAgent && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-2 md:p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-[4px]" onClick={() => setEditingAgent(null)}/>
          <div className="w-full max-w-lg bg-white dark:bg-[#1C1C1E] rounded-[30px] md:rounded-[40px] border border-gray-200 dark:border-white/10 shadow-2xl z-10 flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh]">
            <div className="h-14 md:h-16 px-6 md:px-8 flex items-center justify-between border-b border-gray-100 dark:border-white/5 shrink-0">
              <h3 className="font-bold text-[15px] md:text-[16px] opacity-70">配置智能体</h3>
              <button onClick={() => setEditingAgent(null)} className="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-full text-gray-400 transition-all"><X size={20}/></button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-6 md:space-y-8 scrollbar-hide">
              <div className="flex gap-4 md:gap-6 items-end">
                <div className="w-20 h-20 md:w-24 md:h-24 rounded-[28px] md:rounded-[32px] bg-gray-100 dark:bg-white/5 flex items-center justify-center text-3xl md:text-4xl shadow-inner group relative shrink-0">
                  <input className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" value={editingAgent.avatar} onChange={e => setEditingAgent({...editingAgent, avatar:e.target.value})}/>
                  {editingAgent.avatar}
                </div>
                <div className="flex-1 space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] px-1">代号</label>
                  <input className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl px-4 py-2.5 text-sm font-bold focus:outline-none transition-all" value={editingAgent.name} onChange={e => setEditingAgent({...editingAgent, name:e.target.value})}/>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  className="w-full h-32 md:h-40 bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-[24px] md:rounded-[32px] p-4 md:p-5 text-[13px] leading-relaxed focus:outline-none shadow-inner resize-none" 
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <select 
                    className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl p-3 text-[13px] font-bold focus:outline-none appearance-none" 
                    value={editingAgent.model} 
                    onChange={e => setEditingAgent({...editingAgent, model:e.target.value})}
                  >
                    {MODELS.map(m => <option key={m.id} value={m.id} className="dark:bg-[#1C1C1E] text-black dark:text-white">{m.name}</option>)}
                  </select>
                </div>
                <div className="space-y-3">
                   <div className="flex justify-between px-1">
                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">最大输出字数</label>
                    <span className="text-[10px] font-mono text-blue-500 font-bold">{editingAgent.maxOutputTokens || 800}</span>
                   </div>
                   <input 
                    type="range" 
                    min="100" 
                    ...
                    value={editingAgent.maxOutputTokens || 800} 
                    onChange={e => setEditingAgent({...editingAgent, maxOutputTokens: parseInt(e.target.value)})}
                  />
                </div>
              </div>

              <div className="space-y-3">
                   <div className="flex justify-between px-1"><label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em]">温度控制 (创造力)</label><span className="text-[10px] font-mono">{editingAgent.temperature}</span></div>
                   <input type="range" min="0" max="1" step="0.1" className="w-full h-1 bg-gray-200 dark:bg-white/10 rounded-full accent-blue-500 mt-5" value={editingAgent.temperature} onChange={e => setEditingAgent({...editingAgent, temperature:parseFloat(e.target.value)})}/>
                </div>
            </div>

            <div className="p-6 md:p-8 flex justify-between items-center bg-gray-50 dark:bg-white/[0.01] shrink-0 border-t border-gray-100 dark:border-white/5">
              <button 
                onClick={() => { 
                  if (agents.length <= 1) return alert("请至少保留一个智能体。");
